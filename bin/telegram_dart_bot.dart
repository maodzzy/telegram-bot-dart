...
Future<void> main() async {
    ...
    subscription.onData((data) async {
      ...
      if (data.photo != null) {
        ...
        downloadPhoto(
          ...
          data,
          teledart,
        ).then((value) => teledart.sendMessage(data.chat.id, 'image loaded'));
      }
    });
  });
  ...
  teledart.onMessage(keyword: 'Delete images').listen((message) {
    ...
    deletePhotosDirectory(teledart, message).then(
        (value) => teledart.sendMessage(message.chat.id, 'image deleted'));
  });
}
